...
### 3-5. ローテーション
```bash
python python/auto_post.py refresh-token
```
...
```bash
python python/auto_post.py run             # 通常運用（ニュース収集 + IG 1 件投稿）
python python/auto_post.py collect         # ニュース収集のみ
python python/auto_post.py instagram       # 在庫から IG 投稿のみ（管理画面で手動投稿した記事もここに乗る）
python python/auto_post.py refresh-token   # 長期トークンを更新
```
...
各記事は **タイトル末尾に YYYYMMDD（Asia/Singapore）** をスラッグに含めるので
（例：`new-cold-chain-tech-singapore-20260519`）、同日に同タイトルが再収集
された場合は DB の UNIQUE 制約により構造的にブロックされます（クライアント
仕様「タイトル + 日付など」を実装）。
...
**カテゴリ別プロンプト（[auto_post.py 内 `CATEGORY_IMAGE_PROMPTS`](../python/auto_post.py)）**：
...
- [ ] `pip install -r python/requirements.txt` 完了
- [ ] `python python/auto_post.py collect -v` → `news_articles` に新規行（`tags = {F&B News, Singapore}`）が挿入される
- [ ] `https://thekitchenconnection.sg/news` で記事が表示される
- [ ] `python python/auto_post.py instagram -v` → `@the_kitchen_connection_sg` に投稿される
...
| スラッグ重複防止（タイトル + 日付 YYYYMMDD） | `slugify()` + DB UNIQUE 制約 |
| カテゴリ自動判定（ソース名から industry/regulation/trend/event） | `classify_category()` |
