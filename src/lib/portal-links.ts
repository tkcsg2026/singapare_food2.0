...
export const PORTAL_LINK_FALLBACK_IMG =
 ...
 */
export function mergePortalLinksWithDefaults(
  dbRows: Record<string, unknown>[],
  defaults: readonly (typeof MOCK_PORTAL_LINKS)[number][] = MOCK_PORTAL_LINKS,
  ...
  for (const row of dbRows) {
    ...
    merged.bg_image = bg || PORTAL_LINK_FALLBACK_IMG;
    byUrl.set(u, merged);
  }
...
/** Home carousel needs card fields only — omit long descriptions. */
export function toHomePortalLinkCard(row: Record<string, unknown>) {
  return {
    id: row.id,
    name: row.name,
    name_ja: row.name_ja ?? row.nameJa,
    url: row.url,
    icon: row.icon,
    bg_image: row.bg_image ?? row.bgImage ?? PORTAL_LINK_FALLBACK_IMG,
    sort_order: row.sort_order,
    active: row.active,
  };
}

export async function fetchPublicPortalLinks(
  supabase: SupabaseClient | null,
  options?: { includeInactive?: boolean; homeCardsOnly?: boolean }
) {
  const includeInactive = options?.includeInactive ?? false;
  const homeCardsOnly = options?.homeCardsOnly ?? false;

  if (!supabase) {
    const base = includeInactive
      ? [...MOCK_PORTAL_LINKS]
      : MOCK_PORTAL_LINKS.filter((l) => l.active);
    const sorted = [...base].sort((a, b) => a.sort_order - b.sort_order);
    return homeCardsOnly ? sorted.map((l) => toHomePortalLinkCard(l as Record<string, unknown>)) : sorted;
  }

  const select = "*";

  let query = supabase.from("portal_links").select(select).order("sort_order");
  ...
  const normalized = (data ?? []).map((row: Record<string, unknown>) => {
    ...
    return { ...row, bg_image: bg || PORTAL_LINK_FALLBACK_IMG };
  });

  if (includeInactive) {
    return homeCardsOnly ? normalized.map(toHomePortalLinkCard) : normalized;
  }

  const merged = mergePortalLinksWithDefaults(normalized, MOCK_PORTAL_LINKS, false);
  return homeCardsOnly ? merged.map(toHomePortalLinkCard) : merged;
}
